...
func (_q *ServiceQuery) QuerySystem() *SystemQuery {
...
func (_q *ServiceQuery) QueryVms() *VMQuery {
...
func (_q *ServiceQuery) WithSystem(opts ...func(*SystemQuery)) *ServiceQuery {
	query := (&SystemClient{config: _q.config}).Query()
...
func (_q *ServiceQuery) WithVms(opts ...func(*VMQuery)) *ServiceQuery {
	query := (&VMClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ServiceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Service, error) {
	...
	if query := _q.withSystem; query != nil {
		if err := _q.loadSystem(ctx, query, nodes, nil,
			func(n *Service, e *System) { n.Edges.System = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withVms; query != nil {
		if err := _q.loadVms(ctx, query, nodes,
			func(n *Service) { n.Edges.Vms = []*VM{} },
			func(n *Service, e *VM) { n.Edges.Vms = append(n.Edges.Vms, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ServiceQuery) loadSystem(ctx context.Context, query *SystemQuery, nodes []*Service, init func(*Service), assign func(*Service, *System)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Service)
	for i := range nodes {
		if nodes[i].system_services == nil {
			continue
		}
		fk := *nodes[i].system_services
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "system_services" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ServiceQuery) loadVms(ctx context.Context, query *VMQuery, nodes []*Service, init func(*Service), assign func(*Service, *VM)) error {
	...
	query.withFKs = true
	query.Where(predicate.VM(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(service.VmsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.service_vms
		if fk == nil {
			return fmt.Errorf(`foreign-key "service_vms" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "service_vms" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
