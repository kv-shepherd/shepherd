...
type Service struct {
	...
	// NextInstanceIndex holds the value of the "next_instance_index" field.
	NextInstanceIndex int `json:"next_instance_index,omitempty"`
...
func (e ServiceEdges) SystemOrErr() (*System, error) {
	if e.System != nil {
		return e.System, nil
...
func (e ServiceEdges) VmsOrErr() ([]*VM, error) {
	if e.loadedTypes[1] {
		return e.Vms, nil
	}
...
func (*Service) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case service.FieldID, service.FieldName, service.FieldDescription:
			values[i] = new(sql.NullString)
		case service.FieldCreatedAt, service.FieldUpdatedAt:
			values[i] = new(sql.NullTime)
		case service.ForeignKeys[0]: // system_services
			values[i] = new(sql.NullString)
		default:
	...
	for i := range columns {
		...
		case service.FieldNextInstanceIndex:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field next_instance_index", values[i])
			} else if value.Valid {
				_m.NextInstanceIndex = int(value.Int64)
			}
		case service.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field system_services", values[i])
			} else if value.Valid {
				_m.system_services = new(string)
				*_m.system_services = value.String
			}
		default:
...
// QuerySystem queries the "system" edge of the Service entity.
func (_m *Service) QuerySystem() *SystemQuery {
	return NewServiceClient(_m.config).QuerySystem(_m)
}

// QueryVms queries the "vms" edge of the Service entity.
func (_m *Service) QueryVms() *VMQuery {
	return NewServiceClient(_m.config).QueryVms(_m)
}
...
func (_m *Service) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("next_instance_index=")
	builder.WriteString(fmt.Sprintf("%v", _m.NextInstanceIndex))
	builder.WriteByte(')')
