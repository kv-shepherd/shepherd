...
本文档是 Shepherd 平台所有交互流程的**中文翻译版本**，是前后端和数据库开发的参考文档。
...
| **后端完全哑巴** | 后端存储 `map[string]interface{}`，完全不理解字段含义 |
| **Schema 驱动前端** | 前端根据 Schema 类型自动渲染对应 UI 组件 |
...
│  │    ('role-platform-admin', 'PlatformAdmin', true, '平台管理员-全部权限'),           │       │
...
│  └──────────────────────────────────────────────────────────────────────────────────────────┘ │
│                                                                                              │
│  ┌─ 步骤 2: 配置 Namespace (可选，参考 ADR-0015 §9) ─────────────────────────────────────────┐ │
│  │                                                                                          │ │
│  │  平台职责边界:                                                                            │ │
│  │  - ✅ 可选: 帮助创建 Namespace                                                           │ │
...
│  │  {                                                                                       │ │
│  │      "name": "prod-shop",                                                                │ │
│  │      "labels": { "environment": "prod" }   👈 环境标签决定审批策略和集群匹配               │ │
│  │  }                                                                                       │ │
│  │                                                                                          │ │
...
│  │      approved_at = NOW(),                                                         │       │
│  │      cluster_id = 'cluster-a',             👈 管理员选择的集群                      │       │
│  │      storage_class = 'ceph-rbd',           👈 管理员选择的存储类                    │       │
...
│                                                                                              │
│  🔄 异步任务: 后台 Worker 读取 APPROVED 工单，执行 K8s VM 创建                                 │
│                                                                                              │
└─────────────────────────────────────────────────────────────────────────────────────────────┘
...
│  └──────────────────────────────────────────────────────────────────────────────────┘       │
│                                                                                              │
│  📊 状态流转: PENDING_APPROVAL → REJECTED                                                    │
│  ❌ 无 VM 记录创建                                                                            │
│                                                                                              │
└─────────────────────────────────────────────────────────────────────────────────────────────┘
```
...
> **场景**: 将审计日志集成到企业级监控和 SIEM 系统（Elasticsearch、Datadog、Splunk 等）
...
```json
{
  "logs": [
    {
      "@timestamp": "2026-01-26T10:14:16Z",       // ISO 8601 UTC 时间戳
      "event_id": "log-001",
      "action": "vm.create",                      // 操作类型
      ...
      "context": {
        ...
        "correlation_id": "req-xxx-yyy"          // 关联请求 ID (便于分布式追踪)
      },
...

{
  ...
  "method": "POST",
  "headers": {
    "DD-API-KEY": "${DATADOG_API_KEY}"           // 敏感值使用环境变量
  },
  "filters": {
    "actions": ["*.delete", "approval.*"],       // 仅推送特定操作
    "environments": ["prod"]                      // 仅推送生产环境
  },
  "batch_size": 100,                             // 批量推送数量
  "flush_interval_seconds": 60                   // 推送间隔
}
```
...
| 实践 | 说明 |
|------|------|
| **结构化日志** | 始终使用 JSON 格式，便于查询和分析 |
| **一致的字段名** | 使用统一的字段命名规范 (snake_case) |
| **关联 ID** | 包含 `correlation_id` 便于分布式追踪 |
| **脱敏处理** | 导出前自动脱敏 PII 和敏感数据 |
| **浅层嵌套** | JSON 嵌套最多 2-3 层，优化查询性能 |
| **源端过滤** | 在导出时过滤低价值日志，减少存储成本 |
