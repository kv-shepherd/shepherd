...
>
> **⚠️ ADR 合规要求**:
> - [ADR-0006](../../../../adr/ADR-0006-unified-async-model.md): 所有写操作通过 River Queue；API 返回 **202 Accepted**
> - [ADR-0009](../../../../adr/ADR-0009-domain-event-pattern.md): 领域事件必须在同一事务中创建
> - [ADR-0012](../../../../adr/ADR-0012-hybrid-transaction.md): 原子性 Ent + sqlc 事务
> - [ADR-0015 §10](../../../../adr/ADR-0015-governance-model-v2.md): 重复请求保护
>
> **审计日志 vs 领域事件**:
...
> **⏰ V1 超时策略 (ADR-0015 §11)**:
> - V1 版本不实现待审批请求的自动超时
> - 超过 7 天的陈旧请求在 UI 中以红色标识高亮
> - 管理员手动跟进处理陈旧请求
> - 自动取消/自动升级功能计划在 V2+ 版本实现
...
│  📤 API 响应 (ADR-0006):                                                                     │
│  ┌──────────────────────────────────────────────────────────────────────────────────┐       │
│  │  HTTP/1.1 202 Accepted                      👈 异步操作，非立即返回              │       │
│  │                                                                                    │       │
│  │  {                                                                                 │       │
...
>
> **⚠️ ADR 合规要求**:
> - [ADR-0015 §13](../../../../adr/ADR-0015-governance-model-v2.md): 级联删除约束
> - [ADR-0015 §13.1](../../../../adr/ADR-0015-governance-model-v2.md): 分层确认机制
> - [ADR-0006](../../../../adr/ADR-0006-unified-async-model.md): 通过 River Queue 异步删除
>
> **设计原则**: 删除是**破坏性操作**。多重安全保护防止意外数据丢失。
...
│    ▼             ▼     ▼             ▼     ▼             ▼                                 │
│  ┌─────┐     ┌──────┐  ✅         ❌      ✅         ❌                                    │
│  │test │     │ prod │  通过       失败     通过       失败                                 │
│  └──┬──┘     └──┬───┘             │                   │                                    │
│     │           │                 ▼                   ▼                                    │
│     ▼           ▼        ┌────────────────┐  ┌────────────────┐                            │
│  ┌───────┐  ┌────────┐   │ 错误: 请先删除 │  │ 错误: 请先删除 │                            │
│  │简单    │  │ 输入   │   │ VMs            │  │ Services       │                            │
│  │确认    │  │VM 名称 │   │ ({count} 个)   │  │ ({count} 个)   │                            │
│  └───┬───┘  └───┬────┘   └────────────────┘  └────────────────┘                            │
│      │          │                                                                          │
│      ▼          ▼                                                                          │
│                                                                                              │
...
│  │                                │   │                                                │ │
│  │  👆 简单 confirm=true 参数      │   │  [取消]  [删除]  (输入匹配前禁用)               │ │
│  │                                │   │                                                │ │
│  └────────────────────────────────┘   └────────────────────────────────────────────────┘ │
│                                                                                              │
│  ┌── Service ────────────────────┐   ┌── System ──────────────────────────────────────┐ │
...
│  │  ⚠️ 删除服务?                 │   │  ⚠️ 删除系统?                                   │ │
│  │                                │   │                                                │ │
│  │  服务: shop / redis            │   │  系统: shop                                    │ │
│  │  状态: 所有 VMs 已删除 ✅       │   │  状态: 所有 Services 已删除 ✅                  │ │
│  │                                │   │                                                │ │
│  │  [取消]  [确认删除]             │   │  请输入系统名称确认:                            │ │
│  │                                │   │  [________________________]                    │ │
│  │                                │   │                                                │ │
...
│           │    │           管理员审批队列                                │ │                  │
│           │    │                                                        │ │                  │
│           │    │   [批准] → River Job → K8s 删除 → 标记 DELETED         │ │                  │
│           │    │   [拒绝] → 状态 = REJECTED → 通知用户                  │ │                  │
│           │    └───────────────────────────────────────────────────────┘ │                  │
│           │                                                            │                    │
│           └── System 删除 ──────────────────────────────────────────────┘                  │
...
│           │              立即软删除 (无需审批)                              │                 │
│           │                                                                 │                 │
│           │   系统名称匹配 → status = DELETED → deleted_at = NOW()          │                 │
│           │   👆 System 已无 VMs (Services 为 0)，风险较低                  │                 │
│           └───────────────────────────────────────────────────────────────┘                 │
│                                                                                              │
│  ┌───────────────────────────────────────────────────────────────────────────────────────┐  │
│  │                          阶段 4: 异步执行 (River)                                       │  │
...
│   ├───────────────────────────────────────────────────────────────────────────┤            │
│   │                                                                            │            │
│   │   1. Worker 获取任务 (kind: VMDeleteJob / ServiceDeleteJob)                │            │
│   │   2. 调用 KubeVirt API: kubectl delete vm <name> -n <namespace>            │            │
│   │   3. 等待 K8s 确认                                                          │            │
│   │   4. 更新平台数据库: status = 'DELETED', deleted_at = NOW()                 │            │
│   │   5. 审计日志: 'vm.deleted' / 'service.deleted'                            │            │
│   │   6. 通知用户: "您的资源已被删除"                                           │            │
│   │                                                                            │            │
│   └───────────────────────────────────────────────────────────────────────────┘            │
│                                                                                              │
...
| 实体 | 环境 | 确认方式 | 需要审批 | 原因 |
|------|------|----------|----------|------|
| **VM** | test | `?confirm=true` 查询参数 | ✅ 是 | 测试资源快速删除 |
| **VM** | prod | 在请求体中输入 VM 名称 | ✅ 是 | 生产环境额外安全保护 |
| **Service** | all | `?confirm=true` 查询参数 | ✅ 是 | 必须先删除所有 VMs |
| **System** | all | 在请求体中输入系统名称 | ❌ 否 | 必须先删除所有 Services；输入名称已足够确认 |

> **技术详情**: 参考 [ADR-0015 §13](../../../../adr/ADR-0015-governance-model-v2.md) 了解级联约束实现，以及 [ADR-0015 §13.1](../../../../adr/ADR-0015-governance-model-v2.md) 了解确认 API 设计。
