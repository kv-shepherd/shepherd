...
>
> **⚠️ ADR Compliance**:
> - [ADR-0006](../../adr/ADR-0006-unified-async-model.md): All writes via River Queue; API returns **202 Accepted**
...
> - [ADR-0015 §10](../../adr/ADR-0015-governance-model-v2.md): Duplicate request protection
>
...
>
> **⏰ V1 Timeout Policy (ADR-0015 §11)**:
> - V1 does NOT implement auto-timeout for pending approvals
> - Stale requests (>7 days) highlighted in UI with red indicator
> - Admins manually follow up on stale requests
> - Auto-cancel/auto-escalate planned for V2+
...
│                                                                                              │
│  📋 Pre-submission Validation (ADR-0015 §10 - Duplicate Protection):                        │
│  ┌──────────────────────────────────────────────────────────────────────────────────┐       │
│  │  -- Check for duplicate pending requests (before transaction)                     │       │
│  │  SELECT EXISTS(                                                                   │       │
│  │    SELECT 1 FROM approval_tickets                                                 │       │
│  │    WHERE service_id = :service_id                                                 │       │
│  │      AND operation_type = 'VM_CREATE'                                             │       │
│  │      AND status = 'PENDING_APPROVAL'                                              │       │
│  │  );                                                                               │       │
│  │                                                                                    │       │
│  │  IF exists THEN                                                                   │       │
│  │      RETURN error('DUPLICATE_PENDING_REQUEST',                                    │       │
│  │             'A pending VM create request already exists for this service');       │       │
│  │  END IF;                                                                          │       │
│  └──────────────────────────────────────────────────────────────────────────────────┘       │
│                                                                                              │
...
│  └──────────────────────────────────────────────────────────────────────────────────┘       │
│                                                                                              │
│  📤 API Response (ADR-0006):                                                                 │
│  ┌──────────────────────────────────────────────────────────────────────────────────┐       │
│  │  HTTP/1.1 202 Accepted                      👈 Async operation, not immediate     │       │
│  │                                                                                    │       │
│  │  {                                                                                 │       │
...
>
> **⚠️ ADR Compliance**:
> - [ADR-0015 §13](../../adr/ADR-0015-governance-model-v2.md): Cascade delete constraints
> - [ADR-0015 §13.1](../../adr/ADR-0015-governance-model-v2.md): Tiered confirmation mechanism
> - [ADR-0006](../../adr/ADR-0006-unified-async-model.md): Async deletion via River Queue
>
> **Design Principle**: Deletion is a **destructive operation**. Multiple safety guards prevent accidental data loss.

#### Delete Confirmation Flow Diagram

```
┌─────────────────────────────────────────────────────────────────────────────────────────────┐
│                     Resource Deletion Flow - User Journey                                    │
├─────────────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                              │
│  ┌───────────────────────────────────────────────────────────────────────────────────────┐  │
│  │                          Phase 1: User Initiates Delete                                │  │
│  └───────────────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                              │
│       User clicks [Delete] on VM / Service / System                                          │
│                               │                                                              │
│                               ▼                                                              │
│                   ┌───────────────────────┐                                                 │
│                   │  Check Resource Type   │                                                 │
...
│           ▼                   ▼                   ▼                                         │
│  ┌────────────────┐   ┌────────────────┐   ┌────────────────┐                              │
│  │ Check env type │   │ Check VMs = 0  │   │Check Services=0│                              │
│  └────────┬───────┘   └───────┬────────┘   └───────┬────────┘                              │
│           │                   │                   │                                         │
│    ┌──────┴──────┐     ┌──────┴──────┐     ┌──────┴──────┐                                 │
│    │             │     │             │     │             │                                 │
│    ▼             ▼     ▼             ▼     ▼             ▼                                 │
│  ┌─────┐     ┌──────┐  ✅         ❌      ✅         ❌                                    │
│  │test │     │ prod │  Pass       Fail     Pass       Fail                                 │
│  └──┬──┘     └──┬───┘             │                   │                                    │
│     │           │                 ▼                   ▼                                    │
│     ▼           ▼        ┌────────────────┐  ┌────────────────┐                            │
│  ┌───────┐  ┌────────┐   │ Error: Delete  │  │ Error: Delete  │                            │
│  │Simple │  │  Type  │   │ VMs first      │  │ Services first │                            │
│  │Confirm│  │VM Name │   │ ({count} exist)│  │ ({count} exist)│                            │
│  └───┬───┘  └───┬────┘   └────────────────┘  └────────────────┘                            │
│      │          │                                                                          │
│      ▼          ▼                                                                          │
│                                                                                              │
...
│  ┌── Test VM ───────────────────────┐   ┌── Prod VM ─────────────────────────────────────┐ │
│  │                                   │   │                                                │ │
│  │  ⚠️ Delete VM?                   │   │  ⚠️ Delete Production VM?                       │ │
│  │                                   │   │                                                │ │
│  │  VM: test-shop-shop-redis-01      │   │  VM: prod-shop-shop-redis-01                   │ │
│  │                                   │   │                                                │ │
│  │  [Cancel]  [Confirm Delete]       │   │  To confirm, type the VM name:                 │ │
│  │                                   │   │  [________________________]                    │ │
│  │                                   │   │                                                │ │
│  │  👆 Simple confirm=true param     │   │  [Cancel]  [Delete]  (disabled until match)   │ │
│  │                                   │   │                                                │ │
│  └───────────────────────────────────┘   └────────────────────────────────────────────────┘ │
│                                                                                              │
│  ┌── Service ───────────────────────┐   ┌── System ──────────────────────────────────────┐ │
│  │                                   │   │                                                │ │
│  │  ⚠️ Delete Service?              │   │  ⚠️ Delete System?                              │ │
│  │                                   │   │                                                │ │
│  │  Service: shop / redis            │   │  System: shop                                  │ │
│  │  Status: All VMs deleted ✅       │   │  Status: All Services deleted ✅               │ │
│  │                                   │   │                                                │ │
│  │  [Cancel]  [Confirm Delete]       │   │  To confirm, type the system name:             │ │
│  │                                   │   │  [________________________]                    │ │
│  │                                   │   │                                                │ │
│  │  👆 Simple confirm=true param     │   │  [Cancel]  [Delete]  (disabled until match)   │ │
│  │                                   │   │                                                │ │
...
│           │    │                                                        │ │                  │
│           │    │   [Approve] → River Job → K8s Delete → Mark DELETED    │ │                  │
...
│                       ▼                                                                      │
│           ┌───────────────────────────────────────────────────────────────┐                 │
│           │              Immediate Soft Delete (No Approval)               │                 │
│           │                                                                 │                 │
│           │   System name matched → status = DELETED → deleted_at = NOW()  │                 │
│           │   👆 System has no VMs (Services already 0), less risk         │                 │
│           └───────────────────────────────────────────────────────────────┘                 │
│                                                                                              │
│  ┌───────────────────────────────────────────────────────────────────────────────────────┐  │
│  │                          Phase 4: Async Execution (River)                              │  │
│  └───────────────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                              │
│   On Approval:                                                                              │
│           │                                                                                 │
│           ▼                                                                                 │
│   ┌───────────────────────────────────────────────────────────────────────────┐            │
│   │                        River Queue Processing                              │            │
│   ├───────────────────────────────────────────────────────────────────────────┤            │
│   │                                                                            │            │
│   │   1. Worker picks up job (kind: VMDeleteJob / ServiceDeleteJob)            │            │
│   │   2. Call KubeVirt API: kubectl delete vm <name> -n <namespace>            │            │
│   │   3. Wait for K8s confirmation                                              │            │
│   │   4. Update platform DB: status = 'DELETED', deleted_at = NOW()            │            │
│   │   5. Audit log: 'vm.deleted' / 'service.deleted'                            │            │
│   │   6. Notify user: "Your resource has been deleted"                          │            │
│   │                                                                            │            │
│   └───────────────────────────────────────────────────────────────────────────┘            │
│                                                                                              │
└─────────────────────────────────────────────────────────────────────────────────────────────┘
...
| Entity | Environment | Confirmation Method | Approval Required | Rationale |
|--------|-------------|---------------------|-------------------|-----------|
| **VM** | test | `?confirm=true` query param | ✅ Yes | Quick deletion for test resources |
| **VM** | prod | Type VM name in body | ✅ Yes | Extra safety for production |
| **Service** | all | `?confirm=true` query param | ✅ Yes | Must have 0 VMs first |
| **System** | all | Type system name in body | ❌ No | Must have 0 Services; typing name is sufficient |

> **Technical Details**: See [ADR-0015 §13](../../adr/ADR-0015-governance-model-v2.md) for cascade constraint implementation and [ADR-0015 §13.1](../../adr/ADR-0015-governance-model-v2.md) for confirmation API design.
