...
| Real-cluster live E2E execution evidence | Environment verification | `scripts/run_e2e_live.sh`, ADR-0058, and [live-e2e-validation.md](../operations/live-e2e-validation.md) define the validation path and evidence bundle. This is manual beta/RC, production-readiness, or high-risk provider-change evidence, not a normal alpha patch gate; completion still requires a real PostgreSQL-backed backend and real K8s/KubeVirt cluster run. |
