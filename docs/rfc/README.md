...
| [RFC-0010](./RFC-0010-observability.md) | Runtime Prometheus metrics are accepted by ADR-0054; starter rules, rule tests, runbook-link checks, and Grafana assets are accepted by ADR-0055; optional monitoring deployment packaging is accepted by ADR-0056; HTTP tracing and bounded HTTP/River correlation logs are accepted by ADR-0057; approval/audit business monitoring, OpenTelemetry Collector, Tempo, DB spans, River worker spans, and KubeVirt/provider spans are implemented; frontend tracing, provider log correlation, broad business SLOs, advanced alert routing, and advanced dashboards remain deferred. |
