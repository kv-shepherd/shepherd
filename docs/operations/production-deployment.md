...
| Monitoring image versions | Optional | `PROMETHEUS_IMAGE`, `GRAFANA_IMAGE` |
...
1. Set explicit image versions and Grafana credentials:
...
4. Confirm Prometheus and Grafana:
...
Prometheus and Grafana UI exposure is an operator decision. Do not expose either
UI publicly without local authentication, TLS, and network policy controls.
...
For Docker Compose tracing, `OBSERVABILITY_TRACING_ENABLED=true` must be paired
with the standard OpenTelemetry exporter environment in `.env.prod`, such as
...
| [docker-compose.monitoring.yml](../../deploy/prod/docker-compose.monitoring.yml) | Optional Prometheus and Grafana overlay |
