...
describe('useAdminClustersController', () => {
  ...
  beforeEach(() => {
    ...
    let mutationCall = 0;
    useApiMutationMock.mockImplementation((mutationFn: (req: unknown) => Promise<unknown>) => {
      mutationCall += 1;
      return {
  ...
  it('loads cluster policy via GET when opening the policy modal', async () => {
    ...
    expect(apiGetMock).toHaveBeenCalledWith('/admin/clusters/{cluster_id}/policy', {
      params: { path: { cluster_id: 'cl-1' } },
    });
    expect(policyFormState.setFieldsValue).toHaveBeenCalledWith(expect.objectContaining({
      allow_cpu_overcommit: false,
      allow_gpu: false,
      allowed_storage_classes: ['rook-ceph-block'],
    }));
  ...
  it('submits policy updates via PUT using the selected cluster id', async () => {
    ...
    expect(apiPutMock).toHaveBeenCalledWith('/admin/clusters/{cluster_id}/policy', {
      params: { path: { cluster_id: 'cl-1' } },
      body: {
        allow_cpu_overcommit: false,
        allow_memory_overcommit: true,
