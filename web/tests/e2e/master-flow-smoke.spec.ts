 ...
 */
async function mockMasterFlowBaselineApi(
  page: Page,
  options?: MockMasterFlowOptions,
) {
  await page.route("**/api/v1/**", async (route) => {
        ...
        items: [
          {
            id: "sys-1",
            name: "shop",
            description: "E-commerce core system",
            created_by: "alice",
            created_at: new Date().toISOString(),
          },
        ],
    ...
    if (method === "GET" && path.endsWith("/admin/namespaces")) {
      return json({
        items: [
          { id: "ns-1", name: "prod-shop", environment: "prod", enabled: true },
        ],
        pagination: { page: 1, per_page: 20, total: 1, total_pages: 1 },
      });
    }
...
test.describe("master-flow mock smoke interactions", () => {
  ...
  test("Stage 4.A: user can create a System via modal (POST /systems)", async ({
    ...
    await expect
      .poll(() =>
        captured.some(
          (r) => r.method === "POST" && r.path.endsWith("/systems"),
        ),
      )
      .toBeTruthy();
  });
  ...
  test("Stage 4.A: user can edit System description (PATCH /systems/{id})", async ({
    ...
    await expect
      .poll(() =>
        captured.some(
          (r) => r.method === "PATCH" && /\/systems\/[^/]+$/.test(r.path),
        ),
      )
      .toBeTruthy();
  });
  ...
  test("Stage 4.A: user can delete System with name confirmation (DELETE /systems/{id})", async ({
    ...
    await expect
      .poll(() =>
        captured.some(
          (r) => r.method === "DELETE" && /\/systems\/[^/]+$/.test(r.path),
        ),
      )
      .toBeTruthy();
  });
  ...
  test("Stage 4.B: user can create a Service under a System (POST /systems/{id}/services)", async ({
    ...
    await expect
      .poll(() =>
        captured.some(
          (r) =>
            r.method === "POST" && /\/systems\/[^/]+\/services$/.test(r.path),
        ),
      )
      .toBeTruthy();
  });
  ...
  test("Stage 4.B: user can edit Service description", async ({ page }) => {
    ...
    await expect
      .poll(() =>
        captured.some(
          (r) => r.method === "PATCH" && /\/services\/[^/]+$/.test(r.path),
        ),
      )
      .toBeTruthy();
  });
  ...
  test("Stage 4.B: user can delete Service via Popconfirm", async ({
    ...
    await expect
      .poll(() =>
        captured.some(
          (r) => r.method === "DELETE" && /\/services\/[^/]+$/.test(r.path),
        ),
      )
      .toBeTruthy();
  });
  ...
  test("Stage 5.A: VM request wizard opens and submits POST /vms/request", async ({
    ...
    await selectAntOption(
      page,
      modal.locator(".ant-select").nth(0),
      MOCK_SYSTEM_NAME,
    );
    ...
    await selectAntOption(
      page,
      modal.locator(".ant-select").nth(1),
      MOCK_SERVICE_NAME,
    );
    ...
    await expect
      .poll(() =>
        captured.some(
          (r) => r.method === "POST" && r.path.endsWith("/vms/request"),
        ),
      )
      .toBeTruthy();
  });
  ...
  test("Stage 5.D: VM delete action triggers approval flow (POST /vms/{id}/delete)", async ({
    ...
    const confirmEl = page
      .locator(".ant-popover:visible, .ant-modal-content:visible")
      .first();
    ...
    const confirmBtn = confirmEl
      .getByRole("button", { name: /confirm|ok|delete/i })
      .first();
  ...
  test("Stage 5.E: batch power action from VM page triggers POST /vms/batch/power", async ({
    ...
    await expect
      .poll(() =>
        captured.some(
          (r) => r.method === "POST" && r.path.endsWith("/vms/batch/power"),
        ),
      )
      .toBeTruthy();
  });
  ...
  test("Stage 5.E: batch VM request triggers POST /vms/batch", async ({
    ...
    const batchCountInput = modal.locator(
      "input#vm-request-wizard_batch_count",
    );
    ...
    await expect
      .poll(() =>
        captured.some(
          (r) => r.method === "POST" && r.path.endsWith("/vms/batch"),
        ),
      )
      .toBeTruthy();
  });
  ...
  test("Stage 5.F: notification bell shows unread count and navigates to notifications page", async ({
    ...
    await expect(popover.getByTestId("notification-view-all")).toBeVisible({
      timeout: 5000,
    });
    await Promise.all([
  ...
  test("Stage 5.F: notifications page renders notification items from API", async ({
    ...
    await expect(page.getByText("VM Request Submitted")).toBeVisible({
      timeout: 5000,
    });
  ...
  test("Stage 6: VM console request follows request → open flow", async ({
    ...
    await page.route("**/api/v1/vms/vm-1/console/request", async (route) => {
      ...
      const requestBody = route.request().postDataJSON() as
        | { preferred_console_type?: string }
        | undefined;
      seen.preferredConsoleType = String(
        requestBody?.preferred_console_type ?? "",
      );
      await route.fulfill({
        status: 200,
        contentType: "application/json",
    ...
    await page.getByTestId("vm-action-console-vm-1").click();
    await page.getByRole("radio", { name: /noVNC Console/i }).check();
    ...
    expect(seen.preferredConsoleType).toBe("VNC");
  });
  ...
  test("Stage 6: VM start action triggers POST /vms/{id}/start", async ({
    ...
    await expect
      .poll(() =>
        captured.some(
          (r) => r.method === "POST" && /\/vms\/vm-1\/start$/.test(r.path),
        ),
      )
      .toBeTruthy();
  });
  ...
  test("Stage 6: VM stop action triggers POST /vms/{id}/stop", async ({
    ...
    await expect
      .poll(() =>
        captured.some(
          (r) => r.method === "POST" && /\/vms\/vm-1\/stop$/.test(r.path),
        ),
      )
      .toBeTruthy();
  });
  ...
  test("Stage 5.B: built-in approval task page renders pending tickets", async ({
    ...
    await expect(page.getByTestId("admin-approvals-page")).toBeVisible({
      timeout: 5000,
    });
